...
const accountStore = new AccountStore(() => appHistory.navigate("/settings"));
...
Promise.race([
  ...
  delay(STARTUP_TIMEOUT).then(async () => {
    ...
    throw Object.assign(new Error("Failed to initialize application"), {
      isTimeout: true
    });
  })
]).catch((e) => {
  const failureEvent = Object.assign(new Event("load:failed"), { error: e });
  ...
  appStartupPromise.then(() => {
    serverVersion.then(async (currentVersion) => {
      console.log(
        "Server version was",
        await lastServerVersion,
        "now started late with",
        currentVersion
      );
